...
VPATH = $(SRCFLDR) ; $(OBJFLDR) ; $(SRCFLDR)/text $(SRCFLDR)/dumpsystem $(SRCFLDR)/array $(SRCFLDR)/tree $(SRCFLDR)/stack

SRC := main.cpp Tree.cpp Tree_dump.cpp dumpsystem.cpp database.cpp Array.cpp Akinator.cpp Stack.cpp dump.cpp stack_hash.cpp
OBJ := $(SRC:.cpp=.o)
LIB := iconv
TARGET := $(BINFLDR)/Akinator.exe
